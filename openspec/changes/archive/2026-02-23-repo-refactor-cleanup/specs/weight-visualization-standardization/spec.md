...
The system SHALL expose reusable tensor-extraction utilities for ONNX and PyTorch models that return a normalized tensor metadata structure for visualization.
...
The system MUST preserve quantization-relevant metadata (integer values, scale, zero-point, and dequantized values when available) in the visualization data model.
...
The system SHALL separate visualization business logic (model parsing and tensor preparation) from marimo UI wiring so core behavior can be validated without notebook runtime.
