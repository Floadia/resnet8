...
- Use `mo.stop(not run_button.value)` to gate on the run button
...
- The cell must return `activation_data` (dict mapping layer names to entries with "values" and "shape" keys)
...
def _(
    ...
    mo.stop(
        model_data.get("format") != "pytorch" or not run_button.value,
        mo.md(""),
    )
    ...
    with mo.status.spinner(title="Capturing activations..."):
        try:
            ...
            if input_source.value == "random":
                ...
                _status_msg = "Input: random (1, 32, 32, 3)"
            else:
                try:
                    _sample, _label = load_cifar10_test_sample(
                        _data_dir, int(sample_index.value)
                    )
                    _status_msg = f"Input: CIFAR-10 test[{int(sample_index.value)}] (label={_label})"
            ...
            mo.stop(
                True,
                mo.md(f"**Activation capture failed:** {_e}").callout(kind="danger"),
            )

    activation_data = _activation_data
    activation_status = _status_msg
    mo.md(
...
Important: The cell returns `activation_data` and `activation_status`. Other cells will reference these.
  </action>
  <verify>
...
```python
@app.cell
def _(activation_data, layer_selector, model_data, tensor_entry, view_toggle):
        ...
        if _act is None:
            ...
            for _k, _v in activation_data.items():
                if _k.startswith(_layer + "_") or _k == _layer:
                    ...
                    break
        display_entry = _act
        display_mode = "activations"
    else:
        display_entry = tensor_entry
        display_mode = "weights"
    return display_entry, display_mode
```
...
<task type="auto">
  <name>Task 3: Handle activation_data initialization and edge cases</name>
  <files>playground/weight_visualizer.py</files>
  <action>
Ensure the notebook works correctly in all states: no model loaded, ONNX model loaded, PyTorch model loaded but no inference run, and after inference.

**Edge case: activation_data not yet defined**

Marimo's reactive DAG means cells that reference `activation_data` will error if the activation capture cell hasn't run. Add an initialization cell that provides default empty values, placed BEFORE any cell that references `activation_data`. This cell should depend only on `model_data`:

```python
@app.cell
def _(model_data):
    """Initialize activation data defaults."""
    activation_data = {}
    activation_status = ""
    return activation_data, activation_status
```

IMPORTANT: In Marimo, if two cells both define `activation_data`, the DAG handles this correctly — the capture cell OVERRIDES the defaults when the run button is clicked. However, Marimo does NOT allow two cells to define the same variable. Instead, the activation capture cell in Task 1 must be the ONLY cell defining `activation_data`.

The correct approach: the cells that consume `activation_data` (view_toggle, display_entry resolver) must handle the case where `activation_data` is empty `{}`. The capture cell already returns `activation_data = _activation_data` (an empty dict if nothing was captured). The `mo.stop()` gating in the capture cell means it returns the empty dict `{}` via `activation_data` in the stop output.

Actually, re-examining Marimo's semantics: when a cell has `mo.stop(condition, output)`, the cell's return variables are NOT set — they remain undefined. This means downstream cells that reference `activation_data` WILL fail if the capture cell was stopped.

**Correct fix:** Make the activation capture cell ALWAYS return `activation_data` and `activation_status`, even when stopped. Use a pattern like:

```python
# At the TOP of the capture cell, before the mo.stop():
activation_data = {}
activation_status = ""

_is_pytorch = model_data.get("format") == "pytorch"
if not _is_pytorch or not run_button.value:
    # Return defaults without stopping (so variables are defined)
    pass  # fall through to return
else:
    # ... capture logic that may override activation_data ...
```

This ensures `activation_data` is always defined. Revise the Task 1 capture cell to use this pattern instead of `mo.stop()`. The cell should:
1. Initialize `activation_data = {}` and `activation_status = ""` at the top
2. Use an `if/else` branch instead of `mo.stop()` for the gating logic
3. Inside the else branch, perform the capture and overwrite `activation_data`
4. Always render output: show the success callout only when `activation_data` is non-empty, show the error/info callout when stopped, show nothing when defaults
5. Always return `activation_data, activation_status` at the end

**Similarly for view_toggle cell:** Handle `activation_data` being empty `{}`:
- `_has_activations = bool(activation_data)` works correctly since `bool({}) == False`

**Ensure the display_entry resolver handles missing activation gracefully:**
- When `activation_data` is empty and toggle is "activations", set `display_entry = None`
- Downstream histogram/stats cells already have `mo.stop(display_entry is None)` guards
...
The file's `@app.cell` order determines the visual top-to-bottom layout in the Marimo editor.
  </action>
  <verify>
...
6. Change layer in activation view → histogram updates for new layer
7. Switch model to ONNX → activation data clears, weight view only
8. Run `marimo run playground/weight_visualizer.py` in script mode → no crash
  </verify>
  <done>
Notebook handles all edge cases without errors. Variables are always defined. Empty activation state is handled gracefully. View toggle only appears after successful inference. Script mode works without crashes. All existing weight visualization functionality is preserved (no regressions).
  </done>
</task>
