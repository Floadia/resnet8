...
must_haves:
  ...
  key_links:
    - from: "docs/quantization/03-qlinearmatmul.md"
      ...
      via: "cross-reference for shared patterns"
      pattern: "See QLinearConv|same pattern"
...
<task type="auto">
  <name>Task 1: Create QLinearMatMul documentation</name>
...
3. **Two-Stage Computation Formula**
   - Reference QLinearConv for detailed explanation, show formula for completeness:
...
4. **Comparison to QLinearConv**
   ...
   - Cross-reference: "See QLinearConv for detailed explanation of two-stage computation"
...
4. References QLinearConv: `grep -qi "QLinearConv\|02-qlinearconv" docs/quantization/03-qlinearmatmul.md`
...
- Worked example using ResNet8 FC layer (64→10)
- INT32 accumulator requirement confirmed
- Cross-references to QLinearConv for shared patterns
  </done>
</task>
...
- [ ] Clear cross-reference to QLinearConv documentation
- [ ] ResNet8 FC layer example included (64→10)
- [ ] scripts/validate_qlinearmatmul.py runs successfully (if ONNX model available)
...
2. Documentation builds on QLinearConv, cross-referencing shared patterns
3. Worked example uses actual ResNet8 FC layer values (64→10)
