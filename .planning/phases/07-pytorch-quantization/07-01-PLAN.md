...
files_modified:
  ...
  - models/resnet8_quantized_int8.pt
autonomous: true

must_haves:
  truths:
    - "Quantized PyTorch model exists at models/resnet8_quantized_int8.pt"
    - "Quantized model evaluates successfully on CIFAR-10 test set"
    - "Accuracy delta is reported vs 87.19% baseline"
    - "uint8 support documented (supported or not supported with reason)"
  artifacts:
    ...
    - path: "models/resnet8_quantized_int8.pt"
      provides: "Quantized PyTorch model using fbgemm backend"
  key_links:
...
Output: Quantized PyTorch model (resnet8_quantized_int8.pt) with documented accuracy and uint8 support status
</objective>
...
- `--output` (default: models/resnet8_quantized_int8.pt)
...
<task type="auto">
  ...
  <files>models/resnet8_quantized_int8.pt</files>
  <action>
Run the quantization workflow and evaluate results:

**Step 1: Attempt quantization**
```bash
python scripts/quantize_pytorch.py --model models/resnet8.pt --output models/resnet8_quantized_int8.pt
```
...
Calculate delta: PyTorch int8 accuracy - 87.19%

**Step 4: Document uint8 support status**
fbgemm backend uses quint8 activations + qint8 weights by default. This is effectively "int8 quantization" in PyTorch terminology. Document:
- If pure uint8-only quantization (U8U8 like ONNX Runtime) is possible with fbgemm → create uint8 model
- If not possible → document as "fbgemm backend does not support uint8-only quantization" in script output
...
**Success threshold:** Quantized model accuracy > 80% (allowing for larger drop than ONNX Runtime if onnx2torch model structure causes issues)
  ...
  <verify>
1. `ls -la models/resnet8_quantized_int8.pt` shows file exists
2. `python scripts/evaluate_pytorch.py --model models/resnet8_quantized_int8.pt` runs and reports accuracy
3. Accuracy reported vs 87.19% baseline (delta calculated)
  </verify>
  <done>
resnet8_quantized_int8.pt exists, evaluates successfully with accuracy delta vs 87.19% reported, uint8 support documented
  </done>
</task>
...
1. [x] Quantized PyTorch model exists (resnet8_int8.pt) - verified by `ls models/resnet8_quantized_int8.pt`
...
- models/resnet8_quantized_int8.pt exists and loads without errors
- Quantized model achieves >80% accuracy on CIFAR-10 test set
- Accuracy delta vs 87.19% baseline is documented
- uint8 support status documented (supported with model, or documented as unsupported)
</success_criteria>
